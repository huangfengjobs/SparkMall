...
object MockerOffline {
  ...
  def insertHive(sparkSession: SparkSession, tableName: String, dataFrame: DataFrame): Unit = {
    sparkSession.sql("use " + ConfigUtil.getValue("hive.database"))
  ...
  def mockUserAction() = {
    ...
    val startDate = new SimpleDateFormat("yyyy-MM-dd").parse("2019-06-01")
    val endDate = new SimpleDateFormat("yyyy-MM-dd").parse("2019-06-30")
